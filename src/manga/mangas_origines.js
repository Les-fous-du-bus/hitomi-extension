...
class DefaultExtension extends MProvider {
  ...
  async getPopular(page) {
    try {
      // MangasOrigines uses /catalogues/ (not /manga/) — verified via @homura vs keiyoushi/extensions parity (session 013)
      ...
      try {
        var url2 = BASE_URL + "/" + (page > 1 ? "page/" + page + "/" : "") + "?s=&post_type=wp-manga&m_orderby=trending";
  ...
  async getLatestUpdates(page) {
    try {
      ...
      try {
        var url2 = BASE_URL + "/" + (page > 1 ? "page/" + page + "/" : "") + "?s=&post_type=wp-manga&m_orderby=latest";
        var res2 = await fetchv2(url2, { headers: HEADERS });
        return this._parseMadaraList(res2);
      } catch (e2) {
        return { list: [], hasNextPage: false };
  ...
  async search(query, page, filters) {
    try {
      // Root-level search (site prefixes no segment before /?s=) — parity vs keiyoushi/extensions (session 013)
      var url = BASE_URL + "/" + (page > 1 ? "page/" + page + "/" : "") + "?s=" + encodeURIComponent(query) + "&post_type=wp-manga";
      var res = await fetchv2(url, { headers: HEADERS });
      return this._parseMadaraList(res);
    } catch (e) {
      return { list: [], hasNextPage: false };
