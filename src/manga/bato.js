...
// DEFAULT_LANG : chaîne vide = toutes langues pour browse/popular/latest.
// search() utilise le filtre "Langue" sélectionné par l'utilisateur (getFilterList).
// Note : getPopular() et getLatestUpdates() ne reçoivent pas de filtres depuis Dart —
// impossible de propager la sélection utilisateur. Valeur "" = comportement multilingue
// cohérent avec lang: "multi" déclaré par l'extension.
const DEFAULT_LANG = "";
...
class DefaultExtension extends MProvider {
  ...
  // ─────────────────────────────────────────────
  // PREFERENCE LANGUE (persistée via Storage bridge)
  // ─────────────────────────────────────────────

  async _getLangPref() {
    try {
      const saved = await Storage.get("lang_pref");
      if (saved !== null && saved !== undefined && saved !== "") return saved;
    } catch (_) {}
    return DEFAULT_LANG;
  }

  async _setLangPref(lang) {
    try {
      await Storage.set("lang_pref", lang);
    } catch (_) {}
  }
  ...
  async getPopular(page) {
    const lang = await this._getLangPref();
    const langParam = lang ? `langs=${lang}&` : "";
    const url = `${BASE_URL}/browse?${langParam}sort=views_w&page=${page}`;
  ...
  async getLatestUpdates(page) {
    const lang = await this._getLangPref();
    const langParam = lang ? `langs=${lang}&` : "";
    const url = `${BASE_URL}/browse?${langParam}sort=update&page=${page}`;
    const html = await fetchv2(url, { headers: HEADERS });
    return this._parseBrowse(html);
  }
  ...
  async search(query, page, filters) {
    ...
    if (filters) {
      ...
      if (langFilter?.value !== undefined) {
        lang = langFilter.value;
        // Persister pour getPopular/getLatestUpdates
        await this._setLangPref(lang);
      }
    }
